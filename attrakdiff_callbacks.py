...
def generate_pdf(scores, n_participants, product_name="", ai_text=""):
    ...
    if hq_mean > 1 and pq_val > 1:
        zone = "Souhaité - le produit est perçu comme utile ET attrayant."
    elif hq_mean > 1 and pq_val < 0:
        zone = "Superflu - qualité hédonique élevée mais utilisabilité à améliorer."
    elif hq_mean < 0 and pq_val > 1:
        zone = "Orienté tâche - efficace mais peu stimulant ou identitaire."
    elif hq_mean < 0 and pq_val < 0:
        zone = "Inutile - des améliorations sont nécessaires sur toutes les dimensions."
    else:
    ...
    # --- Items - tableau résumé ---
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(33, 37, 41)
    pdf.cell(0, 8, "Détail par dimension", ln=True)
    pdf.ln(2)
    ...
    for dim in ["PQ", "HQ-S", "HQ-I", "ATT"]:
        ...
        pdf.set_text_color(255, 255, 255)
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(0, 7, f"  {dim} - {DIM_LABELS[dim]}  (score : {scores.get(dim):+.2f})", fill=True, ln=True)
...
def register_callbacks(app):
    ...
    def download_pdf(n, store):
        ...
        # Récupérer le texte IA s'il existe dans le store
...
Sois précis, actionnable et adapté à un public de professionnels UX.
"""
        ...
        return dbc.Alert(
            [html.Strong("Erreur IA : "), error_msg,
             html.Br(),
